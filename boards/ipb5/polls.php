...
class IPB5_Converter_Module_Polls extends Converter_Module_Polls {
	...
	function import()
	{
		...
		// TODO: Verify core_polls table name in IPS5
		...
		while($poll = $this->old_db->fetch_array($query))
		{
			if($poll['tid'] == 0)
			{
				$query2 = $this->old_db->simple_select('forums_topics', 'tid', "poll_state={$poll['pid']}");
				$poll['tid'] = $this->old_db->fetch_field($query2, 'tid');
			}
	...
	function convert_data($data)
	{
		...
		// TODO: Verify core_polls column names (choices, start_date, votes, pid, tid)
